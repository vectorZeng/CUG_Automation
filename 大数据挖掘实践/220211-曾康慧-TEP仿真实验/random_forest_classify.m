%% 故障设置以及预处理
%小波去噪
simout_00t = wdenoise(simout_00);
simout_01t = wdenoise(simout_01);
simout_02t = wdenoise(simout_02);
simout_03t = wdenoise(simout_03);
simout_04t = wdenoise(simout_04);
simout_05t = wdenoise(simout_05);

% 添加标签
simout_00_labeled = [simout_00t, zeros(size(simout_00,1),1)];
simout_01_labeled = [simout_01t, 1*ones(size(simout_00,1),1)];
simout_02_labeled = [simout_02t, 2*ones(size(simout_00,1),1)];
simout_03_labeled = [simout_03t, 3*ones(size(simout_00,1),1)];
simout_04_labeled = [simout_04t, 4*ones(size(simout_00,1),1)];
simout_05_labeled = [simout_05t, 5*ones(size(simout_00,1),1)];

%加标签后的数据集
combined_matrix = [simout_00_labeled; simout_01_labeled;...
    simout_02_labeled; simout_03_labeled;simout_04_labeled; simout_05_labeled];

save labeled combined_matrix
...
% 由 MATLAB 于 2024-06-02 13:30:11 自动生成
