...
% 初始化存储模型的单元数组
models_tree = cell(optimal_clusters, 1);
...
%提取属于当前聚类的数据
    ...
    [RF_pre_Model,~] = treeModel(clusterFeatures, clusterPower);

% 存储模型
    models_tree{k} = RF_pre_Model;
...
% 使用模型进行预测
    ...
    if ~isempty(clusterFeatures)
        test_predictedPower(clusterIndices) = models_tree{k}.predictFcn (clusterFeatures);
...
criteria(3,:) = {{'tree'},normalized_test_mae,normalized_test_rmse,normalized_test_r2};
...
tree_pre_result = test_predictedPower;
