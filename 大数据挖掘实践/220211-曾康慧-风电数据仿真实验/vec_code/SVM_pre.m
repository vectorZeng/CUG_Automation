...
% 初始化存储模型的单元数组
models_SVM = cell(optimal_clusters, 1);
...
%提取属于当前聚类的数据
    ...
    [RF_pre_Model,~] = G_SVM_Model(clusterFeatures, clusterPower);

% 存储模型
    models_SVM{k} = RF_pre_Model;
...
% 使用模型进行预测
    ...
    if ~isempty(clusterFeatures)
        test_predictedPower(clusterIndices) = models_SVM{k}.predictFcn (clusterFeatures);
...
criteria(4,:) = {{'SVM'},normalized_test_mae,normalized_test_rmse,normalized_test_r2};
...
SVM_pre_result = test_predictedPower;
