...
function getScrollTopAdjustment() {
    const FIXED_SCROLL_TOP = 103;
    const DEFAULT_SCROLL_TOP = 10;
    const GLOBAL_NAV_ADJUSTMENT = 35;
    const SUB_NAV_ADJUSTMENT = 87;

    const globalNav = document.getElementById('global_navigation');
    const subNav = document.getElementById('subnav');
    const searchCrumbContainer = $('#search_crumb_container');
    ...
    if (searchCrumbContainer.length > 0) {
        ...
        if ($('.sticky_header_container').is(":visible")) {
            scrollTop = $('.sticky_header_container').height() + $('.section_header').height() + DEFAULT_SCROLL_TOP;
        } else {
            scrollTop = DEFAULT_SCROLL_TOP;
        }
    }

    // Determine any needed navigation adjustment
    if (globalNav) {
        navAdjustment = GLOBAL_NAV_ADJUSTMENT;
    } else if (!globalNav && subNav) {
        navAdjustment = SUB_NAV_ADJUSTMENT;
    }

    // Reset, if we are in Installed doc
    if (typeof getDocCenterType === 'function') {
    ...
    return scrollTop + navAdjustment;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (hash) {
        ...
        if (anchorTarget) {
            ...
            const inAccordion = anchorTarget.closest('.accordion-collapse');
            if (inAccordion) {
