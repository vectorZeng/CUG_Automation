...
void sortstu::disPlayAve()//显示平均分
{
    ...
    else if(what==3)//大学生
    {
        ...
        for (int i = 0; i < stu_lines.length(); ++i) {
            ...
            if (subs.at(0) == "大学生") {
                for (int j = 7; j <= 9; ++j) {
                    double score = subs.at(j).toDouble();
                    stddev[j - 7] += (score - a[2][j - 7] / sum) * (score - a[2][j - 7] / sum);
                }
                // 计算总分的标准差
                double totalScore = subs.at(7).toDouble() + subs.at(8).toDouble() + subs.at(9).toDouble();
                stddev[3] += (totalScore - a[2][3] / sum) * (totalScore - a[2][3] / sum);
            }
        }

        for (int i = 0; i < 4; ++i) {
            stddev[i] = sqrt(stddev[i] / sum);
        }
        ...
        for (int i = 0; i < 4; ++i) {
            passRate[i] = (passRate[i] / sum) * 100;
        }

        // 设置列头
        QStringList headers = {"英语", "程序", "高数", "总分"};
        for (int i = 0; i < headers.size(); ++i) {
            this->model2->setHorizontalHeaderItem(i, new QStandardItem(headers.at(i)));
        }

        // 设置行头
        QStringList rowHeaders = {"平均分", "标准差", "合格率"};
        for (int i = 0; i < rowHeaders.size(); ++i) {
            this->model2->setVerticalHeaderItem(i, new QStandardItem(rowHeaders.at(i)));
        }

        // 填充表格
        for (int i = 0; i < 4; ++i) {
            double avg = a[2][i] / sum;
            QString avgItem = QString::number(avg, 'f', 1);
            this->model2->setItem(0, i, new QStandardItem(avgItem));

            QString stddevItem = QString::number(stddev[i], 'f', 1);
            this->model2->setItem(1, i, new QStandardItem(stddevItem));

            QString passRateItem = QString::number(passRate[i], 'f', 1) + "%";
            this->model2->setItem(2, i, new QStandardItem(passRateItem));
        }
    }
}
